...
convertUsernameToCanonicalform(char *user_name)
{
	...
	/* If user_name is not a valid form <username@domain> then we should probably raise an error. */
	return user_name;
}
/*
...
#define HOSTNAME_LOOKUP_DETAIL(port) \
		...
		case uaMD5:
		case uaPassword:
			if (loginInfo->sspiLen == 0) /* if sspiLen > 0 then GSS auth is already done at this point */
			{
				/*
				* If pg_hba.conf specifies that the entry should be authenticated using
