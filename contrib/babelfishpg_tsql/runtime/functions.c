...
isnumeric(PG_FUNCTION_ARGS)
{
	...
	PG_TRY(); /* Attempt numeric conversion of the string value. */
	{
		(void) DirectFunctionCall3(numeric_in,
						CStringGetDatum(value_str),
						ObjectIdGetDatum(InvalidOid),
						Int32GetDatum(-1));
		result = 1; /* Conversion succeeded. */
	}
	PG_CATCH(); /* Conversion failed. */
	{
	...
	if (!result) /* Attempt money conversion of the string value if numeric conversion failed. */
	{
		PG_TRY();
		{	
			/* Get input function info for money datatype and try conversion to money. */
			getTypeInputInfo((*common_utility_plugin_ptr->get_tsql_datatype_oid)("money"), &typinput, &typioparam);
			(void) OidInputFunctionCall(typinput, value_str, typioparam, -1);
			result = 1; /* Conversion succeeded. */
		}
		PG_CATCH(); /* Conversion failed. */
		{
