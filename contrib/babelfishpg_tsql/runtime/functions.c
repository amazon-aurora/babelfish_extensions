...
search_partition(PG_FUNCTION_ARGS)
{
	...
	/* Raise error if provided partition function doesn't exists in the provided database. */
	...
	/* Get OID of partitin function parameter type. */
	func_param_typoid = (*common_utility_plugin_ptr->get_tsql_datatype_oid) (func_param_typname);

	/* 
	 * Implicitly convert input value to partition function
	 * parameter type and throw error if conversion fails.
	 */
	PG_TRY();
	{
	...
	PG_CATCH();
	{
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("Conversion failed when converting the value to data type %s.",
				func_param_typname)));
	}
	PG_END_TRY();
